do
    local this_uid = 'TANKSYS-EXPERIMENTS-13-KFR-FIXES'
    
    local idx = nil
    for i, m in __active_mods do
        if m.uid == this_uid then
            idx = i
    ...
    if idx then
        table.remove(__active_mods, idx)
    end
		...
		for id, bp in all_bps.Unit do
			...
			if bp.Weapon then
				...
				for i, weapon in bp.Weapon do
                    ...
                    if modConfig.TanksysFixesArc and modConfig.TanksysFixesArc == 'High' then
                        if weapon.BallisticArc and weapon.BallisticArc == 'RULEUBA_LowArc' and not weapon.BallisticArcKeep then
					...
					if weapon.BallisticArc and weapon.BallisticArc == 'RULEUBA_LowArc' then
                        ...
                        if not (weapon.ProjectileLifetime and weapon.ProjectileLifetime ~= 0) then
                            weapon.ProjectileLifetimeUsesMultiplier = 1.7
                        end
					end
